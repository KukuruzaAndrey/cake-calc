...
document.querySelector('#addRecipeRecord').addEventListener('click', (e) => {
  ...
  clone.querySelector('#removeRecipeRecord').addEventListener('click', (e) => {
    if (e.target.parentNode.parentNode.childElementCount > 3) e.target.parentNode.remove()
  })
  recipeRecord.parentNode.insertBefore(clone, document.querySelector('.addRecipe .form-row.col-md-6'))
})

document.querySelector('#removeRecipeRecord').addEventListener('click', (e) => {
  if (e.target.parentNode.parentNode.childElementCount > 3) e.target.parentNode.remove()
})
